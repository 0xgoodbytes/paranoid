...
def work_quick(_searchpath: Path, _hashes_saved: dict[str, HashedFile] | None, _args: argparse.Namespace) -> bool:
    ...
    if not (fileset_new or fileset_deleted or fileset_qchanged):
        print("No quick changes found (size + modification time)")
...
""",
    ...
    parser.add_argument('-q', '--quick',   action="store_true", help="use rsync-style fingerprints (size + modification time)")
    ...
    if multipath:
        ...
        print("Dictionary updates disabled for multi-path runs")
    ...
    return 1 if ret_changes else 0
